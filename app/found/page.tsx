    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50" onClick={handleClose}>
      <main onClick={(e) => e.stopPropagation()} className="w-full max-w-4xl bg-white p-8 rounded-2xl shadow-2xl relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex gap-4 mb-6">
          <button onClick={() => setView('form')} className={`px-4 py-2 rounded-lg font-semibold ${view==='form'? 'bg-green-500 text-white' : 'bg-gray-200'}`}>Report Found</button>
          <button onClick={() => setView('list')} className={`px-4 py-2 rounded-lg font-semibold ${view==='list'? 'bg-green-500 text-white' : 'bg-gray-200'}`}>View Items ({items.length})</button>
        </div>
        ...
        {view === 'form'? (
          ...
          <>
            <h1 className="text-3xl font-bold text-green-500 mb-4">Found Items</h1>
            ...
            <div className="flex gap-2 mb-4">
              ...
              <button onClick={() => setFilter('unclaimed')} className={`px-4 py-2 rounded ${filter==='unclaimed'? 'bg-green-500 text-white' : 'bg-gray-200'}`}>Unclaimed</button>
              <button onClick={() => setFilter('claimed')} className={`px-4 py-2 rounded ${filter==='claimed'? 'bg-green-500 text-white' : 'bg-gray-200'}`}>Claimed</button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredItems.map(item => (
                <div key={item.id} className={`border p-4 rounded-lg ${item.claimed? 'bg-gray-100 opacity-60' : 'bg-white'}`}>
                  ...
                  <h3 className="font-bold text-lg">{item.name} {item.claimed && <span className="text-xs bg-red-500 text-white px-2 py-1 rounded">CLAIMED</span>}</h3>
                  ...
                  {!item.claimed && (
                    <button onClick={() => handleClaim(item.id)} className="mt-2 bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600">
                      Mark as Claimed
                    </button>
                  )}
                </div>
